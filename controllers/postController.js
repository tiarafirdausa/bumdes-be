...
const deleteFile = (filePath, context) => {
  if (!filePath || typeof filePath !== 'string' || !filePath.startsWith("/uploads/posts/")) {
...
exports.createPost = async (req, res) => {
  ...
  try {
    ...
    if (!title || !content || !author_id) {
      if (uploadedFiles) {
        if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "missing fields (featured)");
        if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "missing fields (gallery)");
      }
    ...
    const [postResult] = await connection.query(
      ...
      [
        title, slug, excerpt || null, content, featured_image_path,
        finalMetaTitle, finalMetaDescription, author_id, finalStatus, finalPublishedAt,
      ]
    );
...
    }
    ...
    res.status(201).json({
      id: postId,
      title, slug, excerpt, content, featured_image: featured_image_path,
...
      tags: tags.map(id => parseInt(id)), 
    ...
    if (req.files) {
      if (req.files.featured_image) deleteFile(req.files.featured_image[0].path, "DB failure (featured)");
      if (req.files.gallery_images) deleteMultipleFiles(req.files.gallery_images.map(f => f.path), "DB failure (gallery)");
    }
...
exports.updatePost = async (req, res) => {
  ...
  try {
    ...
    if (oldPost.length === 0) {
      if (uploadedFiles) {
        if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "post not found (featured)");
        if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "post not found (gallery)");
      }
    ...
    if (title !== undefined) {
      ...
      if (existingTitle.length > 0) {
        if (uploadedFiles) {
          if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "duplicate title (featured)");
          if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "duplicate title (gallery)");
        }
    ...
    if (newSlug !== undefined) {
      ...
      if (existingSlug.length > 0) {
        if (uploadedFiles) {
          if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "duplicate slug (featured)");
          if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "duplicate slug (gallery)");
        }
    ...
    if (excerpt !== undefined) { updateFields.push("excerpt = ?"); updateValues.push(excerpt || null); }
    ...
    if (updateFields.length === 0 && categories === undefined && tags === undefined &&
      ...
      clear_gallery_images === undefined && delete_gallery_image_ids === undefined &&
      clear_featured_image === undefined) {
      if (uploadedFiles) {
        if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "no data to update (featured)");
        if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "no data to update (gallery)");
      }
    ...
    if (clear_gallery_images === true) {
      ...
      if (existingGalleryImages.length > 0) {
        const pathsToDelete = existingGalleryImages.map(img => img.image_path);
        deleteMultipleFiles(pathsToDelete, "clear all gallery images");
        await connection.query("DELETE FROM post_gallery_images WHERE post_id = ?", [id]);
        responseBody.gallery_images_cleared = true;
      }
    }

    if (delete_gallery_image_ids && Array.isArray(delete_gallery_image_ids) && delete_gallery_image_ids.length > 0) {
      const placeholders = delete_gallery_image_ids.map(() => '?').join(',');
      ...
      if (imagesToDelete.length > 0) {
        ...
        responseBody.deleted_gallery_image_ids = delete_gallery_image_ids;
      }
    }
  ...
  try {
    ...
    let query = `
      SELECT
          ...
          -- Menambahkan informasi gambar galeri, diurutkan berdasarkan sort_order
    ...
    const processedPosts = posts.map(post => {
      ...
      const gallery_images = post.gallery_images_info ? post.gallery_images_info.split(';').map(img => {
        const [id, image_path, alt_text] = img.split(':');
        return { id: parseInt(id), image_path, alt_text: alt_text === 'null' ? null : alt_text };
      }).filter(img => img.id) : [];
...
exports.getPostById = async (req, res) => {
  try {
    ...
    const gallery_images = post.gallery_images_info ? post.gallery_images_info.split(';').map(img => {
      ...
      return { id: parseInt(imgId), image_path, alt_text: alt_text === 'null' ? null : alt_text };
    }).filter(img => img.id) : [];
...
exports.deletePost = async (req, res) => {
  ...
  try {
    ...
    deleteMultipleFiles(galleryImagePathsToDelete, "post deletion (gallery images)");
