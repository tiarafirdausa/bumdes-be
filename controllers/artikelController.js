...
exports.createArtikel = async (req, res) => { 
    try {
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            res.status(409).json({ error: 'Terjadi duplikasi entri. Judul artikel atau judul SEO mungkin sudah terdaftar.', details: error.message });
        } else {
            res.status(500).json({ error: 'Gagal membuat artikel', details: error.message });
        }
    }
};
...
exports.updateArtikel = async (req, res) => { 
    try {
        ...
        const [oldArtikel] = await db.query('SELECT gambar FROM artikel WHERE id_artikel = ?', [id]);
        ...
        if (judul !== undefined) {
            ...
            if (existingJudul.length > 0) {
                ...
                return res.status(409).json({ error: 'Artikel dengan judul ini sudah ada.' });
            }
        ...
        if (judul !== undefined && !('judul_seo' in req.body)) {
            ...
            if (existingJudulSeo.length > 0) {
                ...
                return res.status(409).json({ error: 'Artikel dengan judul SEO ini sudah ada.' });
            }
            updateFields.push('judul_seo = ?');
        ...
        if (kategori !== undefined) { updateFields.push('kategori = ?'); updateValues.push(kategori); }
        if (tag !== undefined) { updateFields.push('tag = ?'); updateValues.push(tag || null); }
        ...
        const query = `UPDATE artikel SET ${updateFields.join(', ')} WHERE id_artikel = ?`;
        ...
        if (result.affectedRows === 0) {
            ...
            return res.status(404).json({ error: 'Artikel tidak ditemukan atau tidak ada perubahan yang dilakukan.' });
        }

        res.status(200).json({ message: 'Artikel berhasil diperbarui', new_image_path: newGambarPath, ...responseBody });
    } catch (error) {
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            ...
            res.status(500).json({ error: 'Gagal memperbarui artikel', details: error.message });
        }
    }
};
...
exports.deleteArtikel = async (req, res) => {
    try {
        ...
        if (gambarPathToDelete && gambarPathToDelete.startsWith('/public/uploads/artikel')) {
        ...
        res.status(500).json({ error: 'Gagal menghapus artikel', details: error.message });
    }
};