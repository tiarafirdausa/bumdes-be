...
const deleteFile = (filePath, context) => {
  if (!filePath || typeof filePath !== 'string' || !filePath.startsWith("/uploads/pages/")) {
    console.error(`Invalid or suspicious file path for deletion (${context}):`, filePath);
    return;
  }
...
const deleteMultipleFiles = (filePaths, context) => {
  if (!filePaths || !Array.isArray(filePaths)) {
    console.warn(`Attempted to delete multiple files with invalid filePaths array (${context}).`);
    return;
  }
...
exports.createPage = async (req, res) => {
  ...
  try {
    ...
    if (!title || !content || !author_id) {
      if (uploadedFiles) {
        if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "createPage: missing fields (featured)");
        if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "createPage: missing fields (gallery)");
      }
      return res.status(400).json({ error: "Judul, konten, dan ID penulis wajib diisi." });
    }

    const featured_image_path = uploadedFiles && uploadedFiles.featured_image
    ...
    const [pageResult] = await connection.query(
      ...
      [
        ...
        finalMetaTitle, finalMetaDescription, featured_image_path,
        finalStatus, finalPublishedAt,
      ]
    );
    const pageId = pageResult.insertId;

    const insertedGalleryImages = [];
    if (uploadedFiles && uploadedFiles.gallery_images && uploadedFiles.gallery_images.length > 0) {
    ...
    if (req.files) {
      if (req.files.featured_image) deleteFile(req.files.featured_image[0].path, "createPage: DB error (featured)");
      if (req.files.gallery_images) deleteMultipleFiles(req.files.gallery_images.map(f => f.path), "createPage: DB error (gallery)");
    }
...
exports.updatePage = async (req, res) => {
  ...
  try {
    ...
    if (oldPageData.length === 0) {
      if (uploadedFiles) {
        if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "updatePage: page not found (featured)");
        if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "updatePage: page not found (gallery)");
      }
    ...
    if (title !== undefined) {
      ...
      if (existingTitle.length > 0) {
        if (uploadedFiles) { 
          if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "updatePage: duplicate title (featured)");
          if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "updatePage: duplicate title (gallery)");
        }
    ...
    if (newSlug !== undefined) {
      ...
      if (existingSlug.length > 0) {
        if (uploadedFiles) { 
          if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "updatePage: duplicate slug (featured)");
          if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "updatePage: duplicate slug (gallery)");
        }
    ...
    if (author_id !== undefined) { updateFields.push("author_id = ?"); updateValues.push(author_id); }
    ...
    if (calculatedPublishedAt !== null || (published_at !== undefined && published_at === null)) {
    ...
    if (newFeaturedImagePath !== undefined) { 
      ...
      if (oldFeaturedImagePath && oldFeaturedImagePath.startsWith("/uploads/pages/")) {
        deleteFile(oldFeaturedImagePath, "updatePage: old featured image replaced");
      }
    } else if (clear_featured_image === 'true' || clear_featured_image === true) { // Explicitly clear existing image
      updateFields.push("featured_image = ?");
    ...
    if (updateFields.length === 0 &&
        (!uploadedFiles || (!uploadedFiles.gallery_images && !uploadedFiles.featured_image)) &&
        (!delete_gallery_image_ids || delete_gallery_image_ids === '[]') && 
        (!updated_gallery_images_order || updated_gallery_images_order === '[]') &&
        (clear_featured_image !== 'true' && clear_featured_image !== true) 
    ) {
        if (uploadedFiles) {
            if (uploadedFiles.featured_image) deleteFile(uploadedFiles.featured_image[0].path, "updatePage: no data (featured)");
            if (uploadedFiles.gallery_images) deleteMultipleFiles(uploadedFiles.gallery_images.map(f => f.path), "updatePage: no data (gallery)");
        }
    ...
    if (delete_gallery_image_ids && delete_gallery_image_ids !== '[]') {
        ...
        if (idsToDelete.length > 0) {
            ...
            if (imagesToDelete.length > 0) {
                ...
                deleteMultipleFiles(pathsToDelete, "updatePage: delete specific gallery images");
                await connection.query(
    ...
    if (updated_gallery_images_order && updated_gallery_images_order !== '[]') {
        ...
        const updatePromises = orderUpdates.map(item =>
            connection.query("UPDATE page_gallery_images SET sort_order = ? WHERE id = ? AND page_id = ?",
    ...
    if (connection) await connection.rollback(); 
    if (req.files) {
      if (req.files.featured_image) deleteFile(req.files.featured_image[0].path, "updatePage: DB error (featured)");
      if (req.files.gallery_images) deleteMultipleFiles(req.files.gallery_images.map(f => f.path), "updatePage: DB error (gallery)");
    }
    if (error.code === "ER_DUP_ENTRY") {
      res.status(409).json({
        error: "Terjadi duplikasi entri. Judul atau slug halaman mungkin sudah terdaftar.",
...
exports.getPages = async (req, res) => {
  try {
    ...
    if (search) {
      whereClauses.push("(p.title LIKE ? OR p.content LIKE ? OR p.slug LIKE ?)");
    ...
    const allowedSortColumns = ['id', 'title', 'slug', 'created_at', 'updated_at', 'published_at', 'status', 'author_id'];
    let finalSortBy = allowedSortColumns.includes(sort_by) ? `p.${sort_by}` : 'p.created_at';
...
exports.deletePage = async (req, res) => {
  ...
  try {
    ...
    const galleryImagePathsToDelete = galleryImages.map(img => img.image_path);
    ...
    deleteMultipleFiles(galleryImagePathsToDelete, "deletePage: gallery images");
