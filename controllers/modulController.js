...
exports.addModul = async (req, res) => {
  ...
  if (!widget && !home) {
    return res
      .status(400)
      .json({ error: "Setidaknya satu dari Widget atau Home harus dipilih." });
  }
  ...
  try {
    ...
    if (result.affectedRows === 1) {
      const [newModul] = await db.query(
        "SELECT * FROM modul WHERE id_modul = ?",
        [result.insertId]
      );
      res.status(201).json({
...
exports.updateModul = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ error: "Modul tidak ditemukan atau data tidak berubah." });
    }
...
exports.deleteModul = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 1) {
      ...
      res
        .status(404)
        .json({ error: "Modul tidak ditemukan atau gagal dihapus." });
    }
  } catch (error) {
