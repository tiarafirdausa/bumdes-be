...
        const { name } = req.body;
        if (!name) {
            return res.status(400).json({ error: "Nama kategori media tidak boleh kosong." });
        }
...
            return res.status(409).json({ error: "Kategori media dengan nama ini sudah ada." });
        }
...
            res.status(409).json({
                error: "Terjadi duplikasi entri. Kategori media mungkin sudah terdaftar (nama atau slug).",
                details: error.message,
            });
        } else {
            res.status(500).json({ error: "Gagal membuat kategori media", details: error.message });
        }
    }
};
...
        res.status(500).json({ error: "Gagal mengambil kategori media", details: error.message });
    }
};
...
            );
            if (existingCategoryBySlug.length > 0) {
                return res.status(409).json({ error: "Kategori media dengan slug ini sudah ada." });
            }
...
        const query = `UPDATE media_categories SET ${updateFields.join(", ")} WHERE id = ?`;
        updateValues.push(id);
        const [result] = await db.query(query, updateValues);

        if (result.affectedRows === 0) {
            return res.status(404).json({
                error: "Kategori media tidak ditemukan atau tidak ada perubahan yang dilakukan.",
            });
        }
...
        if (error.code === "ER_DUP_ENTRY") {
            res.status(409).json({
                error: "Terjadi duplikasi entri (nama atau slug). Kategori media mungkin sudah terdaftar.",
                details: error.message,
            });
        } else {
            res.status(500).json({ error: "Gagal memperbarui kategori media", details: error.message });
        }
    }
};
...
        res.status(500).json({ error: "Gagal menghapus kategori media", details: error.message });
    }
};