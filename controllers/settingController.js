  ...
  if (oldPath && oldPath.startsWith("/uploads/settings/")) {
    ...
    if (fs.existsSync(fullOldPath)) {
      fs.unlink(fullOldPath, (unlinkErr) => {
        if (unlinkErr)
          console.error(
            "Error deleting old setting image file:",
            fullOldPath,
            unlinkErr
          );
...
exports.getSettings = async (req, res) => {
  try {
    ...
    settingsRows.forEach((setting) => {
      ...
      settingsObject[setting.group][setting.key] = parseSettingValue(
        setting.value,
        setting.type
      );
    });
...
exports.updateSettings = async (req, res) => {
  ...
  try {
    ...
    const currentSettingsMap = new Map(
      currentSettingsRows.map((s) => [s.key, { value: s.value, type: s.type }])
    );
